...
@section('content')
    <h1 class="text-lg text-center font-bold pt-8">Posts from {{ $game->title }}</h1>
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-8">
        @forelse ($gamePosts as $post)
            ...
            <div class="p-4">
                ...
                <div class="flex items-center mt-4">
                    <a href="/user/{{ $post->user->id }}" class="flex items-center gap-2 hover:text-blue-500">
                        <img class="rounded-full w-10 h-10" src="{{ Storage::url($post->user->profile_image) }}" alt="">
        ...
        @empty
            <p class="text-center text-gray-500">No posts from this game yet.</p>
        @endforelse
    </div>
@endsection
